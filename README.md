...
> 当前产品方向：本地优先的 AI 视频前期生产与剪辑交付助手。复杂终剪交给 Premiere Pro、剪映等专业软件；架构调整与实施进度见 [生产工作流重构总文档](docs/production-workflow-refactor.md)。
...
项目概览使用本地项目、镜头、时长和素材数据生成统计与最近项目列表，不包含
会员升级、模板市场、推荐内容等平台化模块。
应用启动时会恢复最近一次打开的本地项目，并默认进入项目概览；只有工作区
没有项目时才显示项目创建页面。

新建项目页支持选择 AI 生成、导入文案、导入脚本或空白项目，并设置项目描述、
视频类型、发布平台、画面比例、目标时长和帧率；这些参数会写入本地项目文件。
创建流程为可点击的六步向导，并补充主题/文案/脚本输入、叙事语气、字幕风格、
自动字幕与素材建议选项和最终确认页；创建界面字号会随窗口宽度自适应。

文案、脚本与分镜、配音和素材页面采用统一三栏工作台：文案支持项目配置、
段落编辑与结构分析；分镜支持镜头计划和提示词编辑；配音按段落管理旁白；
素材页支持搜索、筛选并替换当前镜头素材。
