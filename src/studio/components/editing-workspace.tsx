  ...
  return (
    <div className="edit-app">
      <aside className="edit-sidebar">
        <button className="edit-logo" onClick={onProjects}>
          <b>◆</b>
          <span>
        ...
        <button className="new-project-nav" onClick={onProjects}>
          ＋ 新建项目
        </button>
        <nav>
          {navItems.map(([label, icon]) => (
            <button
              key={label}
              className={label === '剪辑' ? 'active' : ''}
              onClick={
                label === '项目概览'
                  ? onProjects
                  : label === '文案' || label === '脚本与分镜'
                    ? onBack
                    : label === '素材'
                      ? onAssets
                      : label === '导出'
            ...
            >
              <i>{icon}</i>
              <span>{label}</span>
              {['文案', '脚本与分镜', '配音'].includes(label) ? <em>✓</em> : null}
            </button>
          ))}
        </nav>
      ...
      <main className="edit-main">
        <header className="edit-header">
          <div>
            <button onClick={onProjects}>我的项目</button>
            <span>/</span>
          ...
          <div>
            <button className="header-step" onClick={onBack}>
              ← 上一步：素材
            </button>
            <button className="header-next" onClick={onRender}>
              下一步：导出 →
            </button>
          </div>
        </header>
        ...
        <section className="editing-grid">
          <section className="storyboard-pane edit-panel">
            <header>
              ...
              <button onClick={onBack}>自动分镜</button>
            </header>
            <div className="edit-scene-list">
              {project.scenes.map((item, index) => (
                <article
                  ...
                  onDrop={(event) => reorderScenes(event.dataTransfer.getData('sceneId'), item.id)}
                  onClick={() => chooseScene(item.id, index)}
                >
                  <span className="drag-handle">⋮⋮</span>
                  <div className="scene-thumb">
                    <MediaThumb projectId={projectId} path={item.assetPath} type={item.assetType} />
          ...
          <section className="material-pane edit-panel">
            <header>
              <div>
                <strong>镜头 {String(selectedIndex + 1).padStart(2, '0')} · 当前镜头素材</strong>
            ...
            <div className="candidate-list">
              {candidates.slice(0, 4).map((candidate) => (
                ...
                >
                  <MediaThumb projectId={projectId} path={candidate.path} type={candidate.type} />
                  <span>{candidate.name}</span>
                </button>
              ))}
              {!candidates.length ? <p>当前分类还没有候选素材，可打开素材库导入或生成。</p> : null}
            </div>
          ...
          <aside className="properties-pane edit-panel">
            ...
            {inspectorTab === 'properties' ? (
              <div className="property-form">
                ...
                <label>
                  <span>转场</span>
                  <input value={project.style.transition === 'fade' ? '淡入淡出' : '无'} disabled />
                </label>
              </div>
            ) : (
              <div className="property-form">
        ...
        <section className="timeline-panel edit-panel">
          ...
          <div className="track-row music-track">
            <strong>♫ 背景音乐</strong>
            <div className="waveform">
              <span>{project.style.backgroundMusicVolume ? '背景音乐' : '尚未添加背景音乐'}</span>
            </div>
          </div>
